...
import useStyles from './AdminOption.style'
import {
  Box,
  Button,
  CircularProgress,
...
import { categoryApi, optionApi } from '@/utils/api'
...
const AdminOption = ({ cakeFaceId }: Props) => {
  ...
  let handleDeleteEvent = async (index: number) => {
    if (confirm('Xác nhận xóa nội dung')) await DeleteCategory(optionList[index].id)
  }
  ...
  let GetOptionList = async () => {
    dispatch(getOptionList({ params: { cakeFaceId: cakeFaceId } }))
  }

  let CreateNewOption = async () => {
    try {
      if (newName.length === 0) {
      ...
      let params: any = {
        name: newName,
        detail: newDetail,
        cakeFaceId: cakeFaceId,
        isActive: newActive ? '1' : '0',
      }
      if (newFile) {
        params = { ...params, image: newFile }
      }
      let res = await optionApi.create({
        params,
      })
      if (res.data.status) {
        alert('Create new option successfully!')
        await GetOptionList()
        setIsLoading(false)
        setIsShowNewPopup(false)
      } else {
        alert(`Create new option failed!\n${res.data.message}`)
        setIsLoading(false)
        setIsShowNewPopup(false)
      }
    } catch (error) {
  ...
  let UpdateOption = async (id: string, updateOptionDto: UPDATE_OPTION_DTO) => {
    try {
      setIsLoading(true)
      console.log(updateOptionDto)

      let res = await optionApi.update(id, updateOptionDto)
      if (res.data.status) {
        await GetOptionList()
        setEditIndex(-1)
        setIsLoading(false)
  ...
  let handleEditEvent = (index: number, isActive: boolean) => {
    if (editIndex != -1) {
      if (!confirm('Bạn đang chỉnh sửa 1 Tùy chọn khác và chưa lưu. Xác nhận thay đổi Tùy chọn cần chỉnh sửa?')) return
    }
    setEditIndex(index)
    setEditName(optionList[index].name)
    setEditDetail(optionList[index].detail)
    setEditActive(optionList[index].isActive)
    setUpdateFile(null)
    setUpdateThumbnailPreview(optionList[index]?.image)
  }

  let handleUpdate = async (index: number) => {
    let updateOptionDto: UPDATE_OPTION_DTO = {
      params: {
        name: editName,
        detail: editDetail,
    ...
    await UpdateOption(optionList[index].id, updateOptionDto)
  }
