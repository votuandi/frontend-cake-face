...
export default function AdminCakeFace() {
  ...
  let DeleteCakeFace = async (id: string) => {
    // try {
    //   setIsLoading(true)
    //   let res = await cakeFaceList.deleteById(id)
    //   if (res.data.status) {
        ...
        >
          ...
          {isShowNewPopup && (
            ...
            >
              ...
              <input type="file" onChange={handleNewConfigFileChange} style={{ display: 'none' }} id="upload-config" />
              <label htmlFor="upload-config">
                <Button variant="contained" component="span">
                  Chọn file cấu hình
                </Button>
              </label>
              {newConfigFile && <Typography sx={{ color: '#1a1a1a' }}>{newConfigFile.name}</Typography>}
              <FormControl
                sx={{
                  display: 'flex',
                  flexDirection: 'row',
